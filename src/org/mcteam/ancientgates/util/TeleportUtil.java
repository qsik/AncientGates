...
public class TeleportUtil {
	...
	public static void teleportEntity(EntityPortalEvent event, Map<String, String> location) {
		if (Conf.bungeeCordSupport && ((event.getEntityType().isSpawnable() && !EntityUtil.isEchoPet(event.getEntity())) ||
			...
			if (!Conf.useSocketComms || Plugin.serv == null) {
				...
				if (Plugin.instance.getServer().getOnlinePlayers().size() > 0) {
					...
					Plugin.instance.getServer().getOnlinePlayers().iterator().next().sendPluginMessage(Plugin.instance, "BungeeCord", msg.toByteArray());
	...
	public static void teleportPlayer(final Player player, Location location, Boolean teleportEntities, InvBoolean teleportInventory) {
		...
		if (entity != null && teleportEntities && entity instanceof LivingEntity &&
				!(entity instanceof Player) && !EntityUtil.isEchoPet(entity)) {
